...
- **_Note:_ The app (```.app```) should not be directly moved into the ```/Applications``` folder, as the game save files are stored in the same directory as the .app and would be in ```Applications/saves```. Instead, store the .app in a dedicated folder in ```/Applications```, e.g. ```/Applications/MyGame```. This way, the ```saves``` folder would be along the app in its folder and not loose in the ```/Applications``` folder.**
